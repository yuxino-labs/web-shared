...
export function startRegionLanguage(apply, options = {}) {
    const key = "yuxino:site-language:manual:v1";
    const parse = (value) => value === "en" ? "en" : value === "zh" || value === "zh-CN" ? "zh" : undefined;
    ...
    let saved;
    try {
        saved = parse(window.localStorage.getItem(key));
    }
    catch { /* Optional storage. */ }
    let manual = parse(url.searchParams.get("lang")) ?? route ?? saved;
    let current = manual ?? "en";
    let disposed = false;
    let controller;
    let timer;
    const cancel = () => { controller?.abort(); if (timer !== undefined)
        clearTimeout(timer); };
    const remember = (language) => { manual = language; cancel(); try {
        window.localStorage.setItem(key, language);
    }
    catch { /* Explicit URL still works. */ } };
    const select = (language) => { if (disposed || !parse(language))
        return; remember(language); if (language !== current) {
        current = language;
        apply(language);
    } };
    const onClick = (event) => {
    ...
    if (!manual) {
        ...
        const deadline = new Promise(resolve => { timer = setTimeout(() => { controller?.abort(); resolve("en"); }, 1500); });
        void Promise.race([lookup(), deadline]).then(language => { if (timer !== undefined)
            clearTimeout(timer); if (disposed || manual || language === current)
            return; current = language; apply(language); });
    }
    return { select, dispose };
}
